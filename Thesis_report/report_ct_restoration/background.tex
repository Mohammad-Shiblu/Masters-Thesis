...
\begin{equation}
    ...
    \label{eq:psnr}
\end{equation}
where \(M\) is the number of image pixels and \(\mathrm{MAX}\) is the dynamic range assumed in the PSNR calculation. Since all images in this thesis are normalized by \(\mu_{\max}\), their values lie in \([0,1]\), and \(\mathrm{MAX}=1.0\) is therefore used consistently throughout. The LoDoPaB display window \([0, 0.45]\)~\cite{Leuschner2021} is used for visualization only and does not affect the reported PSNR values. The second, \gls{ssim}~\cite{Wang2004}, compares local luminance, contrast, and structure between two images and is more strongly correlated with human perception of image quality than \gls{psnr},
\begin{equation}
