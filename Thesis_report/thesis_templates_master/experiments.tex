...
\begin{equation}
    ...
    \label{eq:exp_rmse}
\end{equation}
All metrics are computed per slice on the $\mu_{\max}$-normalised image and then averaged over the \num{3553} test slices. Before metric computation, model outputs are clamped to $[0,1]$. For PSNR, the dynamic range is set to $\mathrm{MAX}=0.45$, corresponding to the LoDoPaB display maximum used in prior benchmark reporting~\cite{Leuschner2021, Kiss2024}.

All cascade variants are trained for $50$ epochs with AdamW at a learning rate of $3\times 10^{-4}$ using identical seeded corruption draws. Joint losses use equal per-stage weights, $w_1 = w_2 = 0.5$. For the independent-stage variants, the stop-gradient operator at $\hat{\mathbf{x}}_1$ ensures that $\mathcal{L}^{(1)}$ and $\mathcal{L}^{(2)}$ update only the parameters of their respective stages throughout training.
%%─────────────────────────────────────────────────────────
%% Baseline References
%%─────────────────────────────────────────────────────────
...
\begin{table}[!htb]
    ...
    \caption[Baseline references on LoDoPaB-CT under compound corruption.]{%
        ...
        distribution. PSNR and RMSE are computed on the $\mu_{\max}$-normalised image
        with $\mathrm{MAX}=0.45$, following \cite{Leuschner2021}.}
...
Third, RED-CNN improves clearly over BM3D but remains well below the U-Net references, trailing even the smaller U-Net by more than \SI{8}{\decibel} in PSNR. This suggests that, under the present compound-corruption protocol, the U-Net architecture is substantially better suited to preserving and restoring anatomical structure. The two U-Net baselines therefore form the most relevant single-stage references for the cascade comparisons reported below.
%%─────────────────────────────────────────────────────────
...
\Cref{fig:exp_qual_dual,fig:exp_qual_baselines} provide representative visual examples on one moderate slice (index~190) and one more difficult slice with visible metal streaks (index~903). For each case, the corrupted FBP input, the Stage~1 output, the Stage~2 output, and the corresponding ground-truth image are shown in the LoDoPaB display window, with per-slice PSNR and SSIM annotated in the figure panels.

\begin{figure}[!htb]
    \centering
    \includegraphics[width=\linewidth]{figures/results/qual_residual_dual_0190.png}\\[1ex]
    \includegraphics[width=\linewidth]{figures/results/qual_residual_dual_0903.png}
    \caption[Residual cascade with dual-filter input --- representative slices.]{%
        Residual independent-stage cascade with dual-filter input
        (\Cref{eq:meth_alt_input}) on slices~190 (top) and~903 (bottom,
        with visible metal streaks). Columns show the corrupted FBP input,
        the Stage~1 output, the Stage~2 output, and the ground truth.}
    \label{fig:exp_qual_dual}
\end{figure}
...
\begin{figure}[!htb]
    ...
    \begin{minipage}{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figures/results/qual_unet_large_0190.png}\\
        \footnotesize Single-stage U-Net (large), \SI{36.11}{\decibel} mean.
    \end{minipage}\hfill
    \begin{minipage}{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figures/results/qual_bm3d_0190.png}\\
        \footnotesize BM3D on FBP, \SI{22.14}{\decibel} mean.
    \end{minipage}
    \caption[Baseline qualitative comparison on slice~190.]{%
        BM3D suppresses part of the fine-grained noise but leaves the dominant
        structured artefacts largely visible. The single-stage U-Net removes
        both noise and artefact contamination much more effectively and is
        visually close to the best two-stage cascade shown in
        \Cref{fig:exp_qual_dual}.}
    \label{fig:exp_qual_baselines}
\end{figure}

The visual results are consistent with the quantitative comparison. BM3D reduces part of the stochastic noise component, which is in line with its denoising-oriented design, but does not substantially remove the structured degradations introduced by motion inconsistency, rings, and metal streaking. The learned models suppress both noise and artefacts more effectively. The remaining visual difference between the large single-stage U-Net and the best residual cascade is small and appears mainly near challenging streak boundaries, where the residual correction objective is expected to be most useful.
%%─────────────────────────────────────────────────────────
...
develop a strongly specialised corrective role, which helps explain why the
