...
\textbf{(1)~Injection in the sinogram domain.} All physical artefacts considered in this work originate in the acquisition chain rather than in the reconstructed image. Respiratory motion induces projection inconsistencies by shifting the object during acquisition~\cite{LuMackie2002, Mao2007}; ring artefacts arise from detector-specific gain or offset errors~\cite{Sijbers2004, Prell2009}; and metal streaks result from photon starvation along rays passing through highly attenuating implants, which are subsequently spread by the \gls{fbp} ramp filter into the characteristic starburst pattern~\cite{Gjesteby2016}. Injecting these effects directly in the image domain, for example by applying motion blur to a reconstructed slice or inserting an artificial high-intensity object, cannot reproduce their reconstruction-mediated propagation. Sinogram-domain injection is therefore the physically consistent way to model how these artefacts arise and interact with \gls{fbp}~\cite{Mao2007}.

\textbf{(2)~Compound, independently gated mixture.} Rather than training a separate specialist model for each artefact type, we expose a single network to all combinations of corruption. Each of the three physical artefacts is activated independently with a per-sample Bernoulli probability, so that the training distribution contains examples with none, any subset, or all three artefacts simultaneously. Because Poisson noise is already present in every \mbox{LoDoPaB} sinogram (see \Cref{sec:meth_poisson}), every sample is noisy; the physical artefacts are additional overlaid corruptions. This design reflects the clinical setting, in which low-dose noise is ubiquitous whereas specific physical artefacts occur only in some cases.

\textbf{(3)~Physical scale preservation.} \mbox{LoDoPaB} sinograms are stored in post-log form and normalised by $\mu_{\max}=\SI{81.36}{\per\metre}$~\cite{Leuschner2021}, so their values lie nominally in $[0,1]$. All artefacts in the proposed protocol are applied in this $\mu_{\max}$-normalised scale, and the corrupted sinogram is clipped back to $[0,1]$ before reconstruction. No additional $z$-score normalisation is introduced, preserving compatibility with the official \mbox{LoDoPaB} baselines~\cite{Leuschner2021, Kiss2024}.

\Cref{fig:meth_pipeline} summarizes the full pipeline. Starting from a clean \mbox{LoDoPaB} sinogram $\mathbf{s}$, the three physical corruptions are applied sequentially under their respective Bernoulli gates, after which the result is clipped and reconstructed by \gls{fbp}. This yields a corrupted image $\tilde{\mathbf{x}}=\mathcal{R}^{-1}\tilde{\mathbf{s}}$, which serves as the network input. The clean image $\mathbf{x}$ provided by the \mbox{LoDoPaB} ground truth is used as the supervision target.
\begin{figure}[!htb]
    \centering
    \begin{tikzpicture}[
        ...
        \node[block] (input)
            {Clean \mbox{LoDoPaB} sinogram $\mathbf{s}$\\
             \footnotesize post-log, $\mu_{\max}$-normalised; contains $N_0{=}4096$ Poisson noise};
    ...
    \end{tikzpicture}
    \caption[Physics-informed multi-artefact injection pipeline.]{%
        Physics-informed multi-artefact injection pipeline. Clean \mbox{LoDoPaB} sinograms (post-log,
        $\mu_{\max}$-normalised, already containing low-dose Poisson noise at $N_0{=}4096$) pass through three
...
\begin{table}[!htb]
    ...
    \caption[LoDoPaB-CT parallel-beam geometry used in this thesis.]{%
        \mbox{LoDoPaB-CT} parallel-beam geometry used in this thesis~\cite{Leuschner2021}. Sinograms and images are stored in $\mu_{\max}$-normalised form, so that values lie nominally in $[0,1]$; multiplication by $\mu_{\max}$ recovers physical linear attenuation coefficients.}
...
We now turn from the corruption protocol to the restoration models. All architectures studied in this thesis share a common U-Net-style encoder--decoder backbone~\cite{Ronneberger2015} and differ only along three axes: (a)~the number of stages, (b)~the form of the input provided to the second stage, and (c)~the way gradients propagate between stages during training. Isolating these factors allows us to measure the marginal value of additional image-domain capacity under the compound corruption regime defined above.

\subsection{Single-stage U-Net backbone}
\label{subsec:meth_unet}
The backbone is a standard U-Net~\cite{Ronneberger2015} with four downsampling levels and a symmetric upsampling path. Each encoder block consists of two $3{\times}3$ convolutions followed by ReLU activations, and downsampling is performed using $2{\times}2$ max-pooling. The decoder mirrors the encoder, using bilinear upsampling and concatenation-based skip connections at each resolution level. Channel widths follow the conventional progression $(64, 128, 256, 512, 1024)$, and a final $1{\times}1$ convolution maps the top-level decoder feature map to a single-channel image. The resulting single-stage model,
\begin{equation}
...
\subsection{Two-stage cascade with alternative-filter input}
\label{subsec:meth_cascade_input}

A second stage is introduced to allow for further refinement in the image-domain. A naive cascade would provide the second stage only with the first-stage output, which increases processing depth but does not introduce any additional measurement-derived information. Instead, the second stage receives a two-channel input,
\begin{equation}
    \mathbf{u}_2 \;=\; \bigl[\,\hat{\mathbf{x}}_1\;\Vert\;\mathbf{y}^{(\mathrm{alt})}\,\bigr],
    \qquad
    \mathbf{y}^{(\mathrm{alt})} \;=\; \mathcal{R}^{-1}_{h_\mathrm{alt}}\!\bigl(\tilde{\mathbf{s}}\bigr),
    \label{eq:meth_alt_input}
\end{equation}
where $\hat{\mathbf{x}}_1 = f_{\boldsymbol{\theta}_1}(\mathbf{y})$ is the first-stage restored image, $\mathcal{R}^{-1}_{h_\mathrm{alt}}$ denotes \gls{fbp} with an alternative reconstruction filter $h_\mathrm{alt}$ (Hann or Shepp--Logan, rather than the Ram--Lak filter used to produce the original network input $\mathbf{y}$), and $\Vert$ denotes channel-wise concatenation. The motivation is that different reconstruction filters realize different trade-offs between noise suppression and spatial resolution~\cite{KakSlaney1988}: Ram--Lak preserves sharp detail but is more noise-sensitive, whereas Hann and Shepp--Logan produce smoother images at the cost of reduced high-frequency structure. Providing both to the second stage supplies complementary image-domain information without introducing a learned sinogram-domain component. The resulting cascade is therefore not a dual-domain method~\cite{Wang2020, Zhang2022}, but rather a multi-input image-domain cascade.

The first-stage U-Net takes a single-channel input ($\mathbf{y}$), whereas the second-stage U-Net receives a two-channel input ($\mathbf{u}_2$). Apart from this difference in input dimensionality, both stages share the same architecture and channel configuration.
%%─────────────────────────────────────────────────────────
...
$\{\boldsymbol{\theta}_1, \boldsymbol{\theta}_2\}$ are optimised.
...
\begin{equation}
    \hat{\mathbf{x}}_{2}
    \;=\; f_{\boldsymbol{\theta}_{2}}\!\bigl(
        [\,\mathrm{sg}(\hat{\mathbf{x}}_{1})\,\Vert\,\mathbf{y}^{(\mathrm{alt})}\,]
    \bigr),
    \label{eq:meth_ind_input}
\end{equation}
where $\mathrm{sg}(\cdot)$ denotes the stop-gradient operator. Stage~1 is supervised directly
...
The two parameter sets are therefore optimised concurrently but with fully decoupled gradient
signals.
...
\begin{equation}
    \hat{\mathbf{x}}_{2}
    \;=\; f_{\boldsymbol{\theta}_{2}}\!\bigl(
        [\,f_{\boldsymbol{\theta}_{1}}(\mathbf{y})\,\Vert\,\mathbf{y}^{(\mathrm{alt})}\,]
    \bigr).
...
Stage~2 is reformulated to predict an additive correction to stage~1 rather than the image directly:
\begin{equation}
    \hat{\mathbf{x}}_{2} \;=\; \hat{\mathbf{x}}_{1}
        \,+\, f_{\boldsymbol{\theta}_{2}}\!\bigl([\,\hat{\mathbf{x}}_{1}\,\Vert\,\mathbf{y}^{(\mathrm{alt})}\,]\bigr).
    \label{eq:meth_residual}
\end{equation}
In our implementation the residual cascade is trained with the independent-stage protocol ---
...
edge fidelity of the final image. Any difference relative to the independent baseline is therefore
attributable to the combination of the residual parameterization and the gradient/SSIM
supervision~\cite{Zhang2017, He2016}, rather than to gradient-flow effects.
...
both stages share the same U-Net channel widths. This leaves open a separate question about
...
\begin{itemize}
    \item \textbf{Large\,$\to$\,small.} Stage~1 uses the large U-Net (channel widths
        $[64,128,256,512,1024]$, ${\approx}7.18$\,M parameters); Stage~2 uses the small U-Net
        (channel widths $[32,64,128,256]$, ${\approx}1.80$\,M parameters). The hypothesis is
    ...
    \item \textbf{Small\,$\to$\,large.} Stage~1 uses the small U-Net; Stage~2 uses the large
        U-Net. This tests the converse: whether giving Stage~2 more representational capacity
        can compensate for a weaker Stage~1 input.
\end{itemize}
...
\begin{tikzpicture}[
    ...
    \node[io, below=14mm of cat] (yalt)
        {$\mathbf{y}^{(\mathrm{alt})}$\\[-2pt]\tiny Hann/S--L};
...
\begin{tikzpicture}[
    ...
    \node[rectangle, rounded corners=2pt, draw, thick,
          ...
          align=center] (sta)
        {Stage~1\\[-2pt]\tiny Small U-Net ($1.80$\,M)};
    \node[rectangle, rounded corners=2pt, draw, thick,
          fill=teal!20, minimum width=30mm, minimum height=11mm,
          align=center, right=12mm of sta] (stb)
        {Stage~2\\[-2pt]\tiny Large U-Net ($7.18$\,M)};
    \draw[arr] (sta) -- node[above, font=\tiny]{$\hat{\mathbf{x}}_1$} (stb);
\end{tikzpicture}\\[4pt]
...
\caption[Cascade architecture variants.]{%
    ...
    $\hat{\mathbf{x}}_1$ is concatenated ($\Vert$) with the
    alternative-filter reconstruction $\mathbf{y}^{(\mathrm{alt})}$
    (Hann or Shepp--Logan) to form the Stage~2 input $\mathbf{u}_2$.
...
\begin{table}[!htb]
    ...
    \caption[Loss function per cascade variant.]{%
        Loss function used to train each model variant. When a cascade is monitored or optimised jointly, both stages contribute with equal weight \(w_{1}=w_{2}=0.5\). SSIM denotes the structural-similarity loss \(1-\mathrm{SSIM}\)~\cite{Wang2004}, and $\mathcal{L}_{\nabla}$ denotes the finite-difference image-gradient $\ell^{1}$ loss.}
